//! Benchmark: F4 Algorithm on Tiny Systems (WITH AVX512 Feature)
//!
//! Same as f4_tiny.rs but compiled with --features avx512-ifma.
//! **NOTE**: Currently F4 doesn't use AVX512 batch operations automatically.
...
fn bench_f4_cyclic3(c: &mut Criterion) {
    let mut group = c.benchmark_group("f4_cyclic3_avx512");
...
fn bench_f4_cyclic4(c: &mut Criterion) {
    let mut group = c.benchmark_group("f4_cyclic4_avx512");
...
fn bench_f4_katsura3(c: &mut Criterion) {
    let mut group = c.benchmark_group("f4_katsura3_avx512");
    group.sample_size(50);
    group.measurement_time(Duration::from_secs(10));
...
fn bench_f4_katsura4(c: &mut Criterion) {
    let mut group = c.benchmark_group("f4_katsura4_avx512");
    group.sample_size(20);
    group.measurement_time(Duration::from_secs(15));
...
fn bench_avx512_info(c: &mut Criterion) {
    #[cfg(all(feature = "avx512-ifma", target_arch = "x86_64"))]
    {
        c.bench_function("avx512_info", |b| {
            b.iter(|| {
                black_box("AVX512-IFMA enabled (but not used in F4 yet)")
            });
        });
    }
