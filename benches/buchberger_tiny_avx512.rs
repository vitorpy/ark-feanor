//! Benchmark: Buchberger Algorithm on Tiny Systems (WITH AVX512 Feature)
//!
//! Same as buchberger_tiny.rs but compiled with --features avx512-ifma.
//! **NOTE**: Currently Buchberger doesn't use AVX512 batch operations automatically.
//! This benchmark exists to complete the 2×2 comparison matrix.
//!
//! Test systems:
...
/// Benchmark Buchberger on Cyclic-3 (very fast)
fn bench_buchberger_cyclic3(c: &mut Criterion) {
    let mut group = c.benchmark_group("buchberger_cyclic3_avx512");
    group.sample_size(50);
    group.measurement_time(Duration::from_secs(10));

    let field = &*BN254_FR;
    let n_vars = 3;
    let degree_cfg = DegreeCfg::new(10).with_precompute(10);

    group.bench_function("cyclic3_degrevlex_avx512", |b| {
...
/// Benchmark Buchberger on Cyclic-4
fn bench_buchberger_cyclic4(c: &mut Criterion) {
    let mut group = c.benchmark_group("buchberger_cyclic4_avx512");
    ...
    group.bench_function("cyclic4_degrevlex_avx512", |b| {
        let poly_ring = MultivariatePolyRingImpl::new_with_mult_table_ex(
            field, n_vars, degree_cfg, (0, 0), Global
        );
...
/// Benchmark Buchberger on Katsura-3 (very fast)
fn bench_buchberger_katsura3(c: &mut Criterion) {
    let mut group = c.benchmark_group("buchberger_katsura3_avx512");
    group.sample_size(50);
    group.measurement_time(Duration::from_secs(10));
    ...
    group.bench_function("katsura3_degrevlex_avx512", |b| {
        let poly_ring = MultivariatePolyRingImpl::new_with_mult_table_ex(
            field, n_vars, degree_cfg, (0, 0), Global
        );
...
/// Benchmark Buchberger on Katsura-4
fn bench_buchberger_katsura4(c: &mut Criterion) {
    let mut group = c.benchmark_group("buchberger_katsura4_avx512");
    group.sample_size(20);
    group.measurement_time(Duration::from_secs(15));
    ...
    group.bench_function("katsura4_degrevlex_avx512", |b| {
        let poly_ring = MultivariatePolyRingImpl::new_with_mult_table_ex(
            field, n_vars, degree_cfg, (0, 0), Global
        );
...
/// Verify AVX512 feature is enabled
fn bench_avx512_info(c: &mut Criterion) {
    #[cfg(all(feature = "avx512-ifma", target_arch = "x86_64"))]
    {
        c.bench_function("avx512_info", |b| {
            b.iter(|| {
                black_box("AVX512-IFMA enabled (but not used in Buchberger yet)")
            });
        });
    }
...
criterion_group!(
    benches,
    bench_avx512_info,
    bench_buchberger_cyclic3,
    bench_buchberger_cyclic4,
    bench_buchberger_katsura3,
    bench_buchberger_katsura4,
);
