//! Benchmark: Buchberger Algorithm on Tiny Systems (No AVX512)
//!
//! Tests the Buchberger algorithm on very small polynomial systems to measure
//! algorithm overhead and baseline performance without AVX512 optimizations.
//!
//! Test systems:
...
fn bench_buchberger_cyclic3(c: &mut Criterion) {
    let mut group = c.benchmark_group("buchberger_cyclic3");
...
fn bench_buchberger_cyclic4(c: &mut Criterion) {
    let mut group = c.benchmark_group("buchberger_cyclic4");
...
fn bench_buchberger_katsura3(c: &mut Criterion) {
    let mut group = c.benchmark_group("buchberger_katsura3");
    group.sample_size(50);
    group.measurement_time(Duration::from_secs(10));
...
fn bench_buchberger_katsura4(c: &mut Criterion) {
    let mut group = c.benchmark_group("buchberger_katsura4");
    group.sample_size(20);
    group.measurement_time(Duration::from_secs(15));
